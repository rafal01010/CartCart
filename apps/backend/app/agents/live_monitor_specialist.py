...
MONITOR_SPECIALIST_AGENT_NAME = "MonitorSpecialistAgent"


class MonitorSpecialistModelRunner(Protocol):
    async def run(
        self,
        agent: Agent[Any],
        model_input: str,
        *,
        ...
        """Run the SDK monitor specialist and return its raw run result."""


@dataclass
class OpenAIAgentsSDKMonitorSpecialistModelRunner:
    async def run(
        self,
        agent: Agent[Any],
        model_input: str,
...
@dataclass
class MockMonitorSpecialistModelRunner:
...
@dataclass
class LiveMonitorSpecialistAgent:
    settings: Settings
    catalog: AgentCatalog = field(default_factory=build_default_agent_catalog)
    model_runner: MonitorSpecialistModelRunner = field(
        default_factory=OpenAIAgentsSDKMonitorSpecialistModelRunner,
    )
    ...
    async def run(self, input_data: ProductAnalysisAgentInput) -> CategoryAnalysis:
        ...
        configuration = build_openai_agent_run_configuration(
            self.settings,
            agent_name=MONITOR_SPECIALIST_AGENT_NAME,
            run_id=str(input_data.run_id),
        )
        agent = _build_monitor_specialist_agent(configuration.model)
        run_config = RunConfig(
        ...
        self._set_activity(
            "model_monitor_analysis_completed",
            input_data,
            route,
            analysis,
        )
        ...
        self._workbench_activity = (
            {
                ...
                "status": status,
                "input": {
                    "agent": MONITOR_SPECIALIST_AGENT_NAME,
                    "allowed_tools": [],
...
def _build_monitor_specialist_agent(model: str) -> Agent[Any]:
    return Agent(
        name="CartCartMonitorSpecialistAgent",
        model=model,
        model_settings=ModelSettings(
            temperature=0,
            max_tokens=1200,
        ...
        instructions=(
            "Analyze exactly one computer monitor/display candidate for fit "
            "against the shopper's brief and return only a structured "
            "CategoryAnalysis. This specialist is only for monitor-scoped "
            ...
            "Cover monitor-specific fit: panel type, resolution, refresh rate, "
            "ergonomics or stand adjustability, ports and device compatibility, "
            "movie/coding/gaming tradeoffs when relevant, listing caveats, and "
            "evidence gaps. If evidence is weak or sparse, return lower "
            "confidence and plain limitations. Do not analyze non-monitor "
            "products as monitors, rank final recommendations, browse, call "
...
def _normalize_category_analysis(
    ...
    if analysis.product_id != input_data.product.product_id:
        raise ValueError("monitor analysis output used an unknown product ID.")
    ...
    if unknown_listing_ids:
        raise ValueError("monitor analysis output used unknown listing IDs.")
    ...
    if unknown_evidence_ids:
        raise ValueError("monitor analysis output used unknown evidence IDs.")

    known_source_ids = _known_source_ids(input_data)
    unknown_source_ids = set(analysis.source_ids) - known_source_ids
    if unknown_source_ids:
        raise ValueError("monitor analysis output used unknown source IDs.")
    ...
    if input_data.evidence and not source_ids:
        raise ValueError("monitor analysis output did not preserve source IDs.")
...
def _validate_analysis_policy(
    ...
    if not analysis.strengths:
        raise ValueError("monitor analysis output must include fit strengths.")
    if not analysis.weaknesses:
        raise ValueError("monitor analysis output must include tradeoffs or gaps.")

    route = _route_for_input(input_data, catalog)
    if not _is_monitor_route(route, catalog):
        raise ValueError("monitor analysis received a non-monitor input.")
    if not _is_monitor_text(analysis.category, catalog):
        raise ValueError("monitor analysis output used a non-monitor category.")
    ...
    if any(_contains_forbidden_text(text) for text in texts):
        raise ValueError("monitor analysis output included forbidden text.")
    if not _mentions_required_monitor_coverage(analysis):
        raise ValueError("monitor analysis omitted required monitor-specific coverage.")
...
def _fallback_for_non_monitor(
    ...
    return _generic_fallback_analysis(input_data), "generic_fallback_non_monitor"
...
def _technology_domain_fallback_analysis(
    ...
    if _is_monitor_route(route, catalog):
        strengths = (
            *strengths,
            "A technology-domain fallback is available when monitor-specialist output is unavailable.",
        )
    ...
    return CategoryAnalysis(
        ...
        weaknesses=(
            ...
            "Specs, warranty, accessory, and connectivity caveats should stay "
            "unknown unless source evidence confirms them.",
        ),
        warnings=warnings,
...
def _monitor_specific_analysis(input_data: ProductAnalysisAgentInput) -> CategoryAnalysis:
    ...
    if weak_or_sparse:
        warnings = (
            "Only limited monitor evidence was supplied, so missing specs should "
            "stay unknown until a source confirms them.",
        )
    ...
    return CategoryAnalysis(
        ...
        category=category,
        fit_summary=(
            f"Monitor-specific fit analysis for {product_name} based on supplied "
            "source evidence, covering panel type, resolution, refresh rate, "
            "ergonomics, ports, and practical coding/movie tradeoffs."
        ),
        strengths=(
            "Panel type and resolution evidence provide the main source-backed check for text clarity, viewing angles, and movie contrast.",
            "Refresh rate and ports evidence help screen motion handling and device compatibility before comparison.",
            "Ergonomics evidence such as stand adjustment or VESA support matters for long coding sessions.",
        ),
        weaknesses=(
            "Monitor tradeoffs remain around panel type, 1440p resolution fit, refresh rate, ergonomics, and ports if any source is incomplete.",
            "A separate seller and listing trust check is still needed before treating the listing as safe to buy.",
        ),
        warnings=warnings,
        confidence=_confidence(
            score,
            "Confidence is based only on supplied monitor evidence and specialist fit rules.",
        ),
        evidence_ids=_fallback_evidence_ids(input_data),
        source_ids=_fallback_source_ids(input_data),
    )
...
def _is_monitor_route(route: ProductAnalysisRoute, catalog: AgentCatalog) -> bool:
    return route.agent_path == (
        catalog.technology_domain_agent_name,
        MONITOR_SPECIALIST_AGENT_NAME,
    )
...
def _is_monitor_text(text: str, catalog: AgentCatalog) -> bool:
    return _is_monitor_route(catalog.route_product_analysis(text), catalog)
...
def _mentions_required_monitor_coverage(analysis: CategoryAnalysis) -> bool:
    combined = " ".join(
        (
            analysis.fit_summary,
            *analysis.strengths,
    ...
    required_groups = (
        ...
        ("resolution", "1440p", "qhd", "4k", "uhd"),
        ("refresh", "hz"),
        ("ergonomic", "ergonomics", "stand", "height", "tilt", "vesa"),
        ("port", "ports", "hdmi", "displayport", "usb"),
        ("tradeoff", "tradeoffs"),
    )
