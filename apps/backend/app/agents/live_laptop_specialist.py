...
LAPTOP_SPECIALIST_AGENT_NAME = "LaptopSpecialistAgent"


class LaptopSpecialistModelRunner(Protocol):
    async def run(
        self,
        agent: Agent[Any],
        model_input: str,
        *,
        ...
        """Run the SDK laptop specialist and return its raw run result."""


@dataclass
class OpenAIAgentsSDKLaptopSpecialistModelRunner:
    async def run(
        self,
        agent: Agent[Any],
        model_input: str,
...
@dataclass
class MockLaptopSpecialistModelRunner:
...
@dataclass
class LiveLaptopSpecialistAgent:
    settings: Settings
    catalog: AgentCatalog = field(default_factory=build_default_agent_catalog)
    model_runner: LaptopSpecialistModelRunner = field(
        default_factory=OpenAIAgentsSDKLaptopSpecialistModelRunner,
    )
    ...
    async def run(self, input_data: ProductAnalysisAgentInput) -> CategoryAnalysis:
        ...
        if not _is_laptop_route(route, self.catalog):
            analysis, status = _fallback_for_non_laptop(input_data, route, self.catalog)
            self._set_activity(status, input_data, route, analysis)
            return analysis

        configuration = build_openai_agent_run_configuration(
            self.settings,
            agent_name=LAPTOP_SPECIALIST_AGENT_NAME,
            run_id=str(input_data.run_id),
        )
        agent = _build_laptop_specialist_agent(configuration.model)
        run_config = RunConfig(
        ...
        self._set_activity(
            "model_laptop_analysis_completed",
            input_data,
            route,
            analysis,
        )
        ...
        self._workbench_activity = (
            {
                ...
                "status": status,
                "input": {
                    "agent": LAPTOP_SPECIALIST_AGENT_NAME,
                    "allowed_tools": [],
...
def _build_laptop_specialist_agent(model: str) -> Agent[Any]:
    return Agent(
        name="CartCartLaptopSpecialistAgent",
        model=model,
        model_settings=ModelSettings(
            temperature=0,
            max_tokens=1200,
        ...
        instructions=(
            "Analyze exactly one laptop or notebook candidate for fit against "
            "the shopper's brief and return only a structured CategoryAnalysis. "
            "This specialist is only for laptop-scoped products routed through "
            ...
            "laptop-specific fit: CPU or processor, RAM or memory, storage, "
            "battery life and charging, display, ports and device compatibility, "
            "weight or portability, upgradeability or repairability, listing "
            "caveats, and evidence gaps. If evidence is weak or sparse, return "
            "lower confidence and plain limitations. Do not analyze non-laptop "
            "products as laptops, rank final recommendations, browse, call "
...
def _normalize_category_analysis(
    ...
    if analysis.product_id != input_data.product.product_id:
        raise ValueError("laptop analysis output used an unknown product ID.")
    ...
    if unknown_listing_ids:
        raise ValueError("laptop analysis output used unknown listing IDs.")
    ...
    if unknown_evidence_ids:
        raise ValueError("laptop analysis output used unknown evidence IDs.")

    known_source_ids = _known_source_ids(input_data)
    unknown_source_ids = set(analysis.source_ids) - known_source_ids
    if unknown_source_ids:
        raise ValueError("laptop analysis output used unknown source IDs.")
    ...
    if input_data.evidence and not source_ids:
        raise ValueError("laptop analysis output did not preserve source IDs.")
...
def _validate_analysis_policy(
    ...
    if not analysis.strengths:
        raise ValueError("laptop analysis output must include fit strengths.")
    if not analysis.weaknesses:
        raise ValueError("laptop analysis output must include tradeoffs or gaps.")

    route = _route_for_input(input_data, catalog)
    if not _is_laptop_route(route, catalog):
        raise ValueError("laptop analysis received a non-laptop input.")
    if not _is_laptop_text(analysis.category, catalog):
        raise ValueError("laptop analysis output used a non-laptop category.")
    ...
    if any(_contains_forbidden_text(text) for text in texts):
        raise ValueError("laptop analysis output included forbidden text.")
    if not _mentions_required_laptop_coverage(analysis):
        raise ValueError("laptop analysis omitted required laptop-specific coverage.")
...
def _fallback_for_non_laptop(
    ...
    return _generic_fallback_analysis(input_data), "generic_fallback_non_laptop"
...
def _technology_domain_fallback_analysis(
    ...
    if _is_laptop_route(route, catalog):
        strengths = (
            *strengths,
            "A technology-domain fallback is available when laptop-focused output is unavailable.",
        )
...
def _laptop_specific_analysis(input_data: ProductAnalysisAgentInput) -> CategoryAnalysis:
    ...
    if weak_or_sparse:
        warnings = (
            "Only limited laptop evidence was supplied, so missing CPU, RAM, "
            "storage, battery, display, port, weight, or upgradeability claims "
            "should stay unknown.",
        )
    ...
    return CategoryAnalysis(
        ...
        category=category,
        fit_summary=(
            f"Laptop-specific fit analysis for {product_name} based on supplied "
            "source evidence, covering CPU, RAM, storage, battery, display, "
            "ports, weight, upgradeability, and practical student portability "
            "tradeoffs."
        ),
        strengths=(
            "CPU, RAM, and storage evidence provide the main source-backed check for performance and day-to-day headroom.",
            "Battery, display, and weight evidence help screen whether the laptop is practical to carry and use in class.",
            "Ports and upgradeability evidence help flag accessory needs, repairability, and long-term flexibility.",
        ),
        weaknesses=(
            "Laptop tradeoffs remain around CPU performance, RAM, storage, battery life, display quality, ports, weight, and upgradeability if any source is incomplete.",
            "A separate seller and listing trust check is still needed before treating the listing as safe to buy.",
        ),
        warnings=warnings,
        confidence=_confidence(
            score,
            "Confidence is based only on supplied laptop evidence and specialist fit rules.",
        ),
        evidence_ids=_fallback_evidence_ids(input_data),
        source_ids=_fallback_source_ids(input_data),
    )
...
def _is_laptop_route(route: ProductAnalysisRoute, catalog: AgentCatalog) -> bool:
    return route.agent_path == (
        catalog.technology_domain_agent_name,
        LAPTOP_SPECIALIST_AGENT_NAME,
    )
...
def _is_laptop_text(text: str, catalog: AgentCatalog) -> bool:
    return _is_laptop_route(catalog.route_product_analysis(text), catalog)
...
def _mentions_required_laptop_coverage(analysis: CategoryAnalysis) -> bool:
    combined = " ".join(
        (
            analysis.fit_summary,
            *analysis.strengths,
    ...
    required_groups = (
        ("cpu", "processor", "chip", "performance"),
        ("ram", "memory"),
        ("storage", "ssd", "drive"),
        ("battery", "runtime", "charging", "charge", "wh"),
        ("display", "screen", "resolution", "brightness", "panel"),
        ("port", "ports", "usb", "hdmi", "thunderbolt"),
        ("weight", "portable", "portability", "kg", "lb", "pound", "carry"),
        (
            "upgrade",
            "upgradeability",
            "repair",
