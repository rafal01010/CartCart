...
SMARTWATCH_SPECIALIST_AGENT_NAME = "SmartwatchSpecialistAgent"


class SmartwatchSpecialistModelRunner(Protocol):
    async def run(
        self,
        agent: Agent[Any],
        model_input: str,
        *,
        ...
        """Run the SDK smartwatch specialist and return its raw run result."""


@dataclass
class OpenAIAgentsSDKSmartwatchSpecialistModelRunner:
    async def run(
        self,
        agent: Agent[Any],
        model_input: str,
...
@dataclass
class MockSmartwatchSpecialistModelRunner:
...
@dataclass
class LiveSmartwatchSpecialistAgent:
    settings: Settings
    catalog: AgentCatalog = field(default_factory=build_default_agent_catalog)
    model_runner: SmartwatchSpecialistModelRunner = field(
        default_factory=OpenAIAgentsSDKSmartwatchSpecialistModelRunner,
    )
    ...
    async def run(self, input_data: ProductAnalysisAgentInput) -> CategoryAnalysis:
        ...
        if not _is_smartwatch_route(route, self.catalog):
            analysis, status = _fallback_for_non_smartwatch(
                input_data,
                route,
                self.catalog,
            )
        ...
        configuration = build_openai_agent_run_configuration(
            self.settings,
            agent_name=SMARTWATCH_SPECIALIST_AGENT_NAME,
        ...
        self._set_activity(
            "model_smartwatch_analysis_completed",
            input_data,
            route,
            analysis,
        )
        ...
        self._workbench_activity = (
            {
                ...
                "status": status,
                "input": {
                    "agent": SMARTWATCH_SPECIALIST_AGENT_NAME,
                    "allowed_tools": [],
...
def _build_smartwatch_specialist_agent(model: str) -> Agent[Any]:
    return Agent(
        ...
        instructions=(
            "Analyze exactly one smartwatch or smart watch candidate for fit against "
            "the shopper's brief and return only a structured CategoryAnalysis. "
            "This specialist is only for smartwatch-scoped products routed through "
            "the technology domain. Use only supplied product, listing, and "
            ...
            "smartwatch-specific fit: phone compatibility for Android or iPhone, "
            "health and fitness sensors, battery life and charging cadence, "
            "durability and water resistance, app ecosystem and ecosystem lock-in, "
            "listing caveats, and evidence gaps. If evidence is weak or sparse, "
            "return lower confidence and plain limitations. Do not analyze "
            "non-smartwatch products as smartwatches, rank final recommendations, "
            "browse, call tools, or expose agents, providers, prompts, traces, "
            "policies, or schemas to shoppers."
        ),
        tools=[],
    ...
    return json.dumps(
        {
            ...
            "allowed_routes": {
                ...
                "smartwatch_specialist_agent_name": SMARTWATCH_SPECIALIST_AGENT_NAME,
                "product_category_routes": catalog.product_category_routes,
            },
            "declared_route": route.model_dump(mode="json"),
            "required_smartwatch_coverage": (
                "phone compatibility",
                "health sensors",
                "battery",
                "durability",
...
def _normalize_category_analysis(
    ...
    if analysis.product_id != input_data.product.product_id:
        raise ValueError("smartwatch analysis output used an unknown product ID.")
    ...
    if unknown_listing_ids:
        raise ValueError("smartwatch analysis output used unknown listing IDs.")
    ...
    if unknown_evidence_ids:
        raise ValueError("smartwatch analysis output used unknown evidence IDs.")

    known_source_ids = _known_source_ids(input_data)
    unknown_source_ids = set(analysis.source_ids) - known_source_ids
    if unknown_source_ids:
        raise ValueError("smartwatch analysis output used unknown source IDs.")
    ...
    if input_data.evidence and not source_ids:
        raise ValueError("smartwatch analysis output did not preserve source IDs.")
...
def _validate_analysis_policy(
    ...
    if not analysis.strengths:
        raise ValueError("smartwatch analysis output must include fit strengths.")
    if not analysis.weaknesses:
        raise ValueError("smartwatch analysis output must include tradeoffs or gaps.")

    route = _route_for_input(input_data, catalog)
    if not _is_smartwatch_route(route, catalog):
        raise ValueError("smartwatch analysis received a non-smartwatch input.")
    if not _is_smartwatch_text(analysis.category, catalog):
        raise ValueError("smartwatch analysis output used a non-smartwatch category.")
    ...
    if any(_contains_forbidden_text(text) for text in texts):
        raise ValueError("smartwatch analysis output included forbidden text.")
    if not _mentions_required_smartwatch_coverage(analysis):
        raise ValueError(
            "smartwatch analysis omitted required smartwatch-specific coverage."
        )
...
def _fallback_for_non_smartwatch(
    input_data: ProductAnalysisAgentInput,
    route: ProductAnalysisRoute,
    catalog: AgentCatalog,
    ...
    return _generic_fallback_analysis(input_data), "generic_fallback_non_smartwatch"
...
def _technology_domain_fallback_analysis(
    ...
    if _is_smartwatch_route(route, catalog):
        strengths = (
            *strengths,
            "A technology-domain fallback is available when smartwatch-focused "
    ...
    return CategoryAnalysis(
        ...
        confidence=_confidence(
            score,
            "Confidence is based only on supplied evidence and broad technology "
            "fit rules.",
        ),
...
def _smartwatch_specific_analysis(
    input_data: ProductAnalysisAgentInput,
) -> CategoryAnalysis:
    weak_or_sparse = _has_weak_or_sparse_evidence(input_data)
    category = _analysis_category(input_data)
    ...
    if weak_or_sparse:
        warnings = (
            "Only limited smartwatch evidence was supplied, so missing phone "
            "compatibility, health-sensor, battery, durability, water-resistance, "
            "and app-ecosystem claims should stay unknown.",
        )
    ...
    return CategoryAnalysis(
        ...
        category=category,
        fit_summary=(
            f"Smartwatch-specific fit analysis for {product_name} based on "
            "supplied source evidence, covering phone compatibility, health "
            "sensors, battery life, durability, water resistance, and app ecosystem."
        ),
        strengths=(
            "Phone compatibility evidence helps screen Android or iPhone fit "
            "before comparing deeper features.",
            "Health sensor and fitness evidence provide the main source-backed "
            "check for workout, heart-rate, GPS, sleep, and safety claims.",
            "Battery, durability, water-resistance, and app-ecosystem evidence "
            "help avoid a watch that fits on specs but fails in daily use.",
        ),
        weaknesses=(
            "Smartwatch tradeoffs remain around phone compatibility, sensor "
            "accuracy, battery life, durability, water resistance, and app "
            "ecosystem if any source is incomplete.",
            "A separate seller and listing trust check is still needed before "
            "treating the listing as safe to buy.",
        ),
        warnings=warnings,
        confidence=_confidence(
            score,
            "Confidence is based only on supplied smartwatch evidence and "
            "specialist fit rules.",
        ),
        evidence_ids=_fallback_evidence_ids(input_data),
        source_ids=_fallback_source_ids(input_data),
    )
...
def _fallback_strengths(input_data: ProductAnalysisAgentInput) -> tuple[str, ...]:
    ...
    if input_data.listings:
        strengths.append(
            "A concrete listing is available for later price, availability, and "
            "seller checks."
        )
    if input_data.brief.constraints or input_data.brief.preferences:
        strengths.append(
            "The shopper's stated needs can be compared against the candidate's "
            "known details."
        )
    return tuple(strengths) or (
        "The candidate can be screened against the shopper's request, but evidence "
        "is minimal.",
    )
...
def _mock_analysis_from_model_input(model_input: str) -> CategoryAnalysis:
    ...
    return _smartwatch_specific_analysis(input_data)
...
def _is_smartwatch_route(route: ProductAnalysisRoute, catalog: AgentCatalog) -> bool:
    return route.agent_path == (
        catalog.technology_domain_agent_name,
        SMARTWATCH_SPECIALIST_AGENT_NAME,
    )
...
def _is_smartwatch_text(text: str, catalog: AgentCatalog) -> bool:
    return _is_smartwatch_route(catalog.route_product_analysis(text), catalog)
...
def _mentions_required_smartwatch_coverage(analysis: CategoryAnalysis) -> bool:
    combined = " ".join(
        (
            analysis.fit_summary,
            *analysis.strengths,
    ...
    required_groups = (
        ("phone compatibility", "android", "iphone", "ios", "wear os", "watchos"),
        ("health sensor", "heart rate", "gps", "spo2", "ecg", "sleep", "fitness"),
        ("battery", "charging", "runtime", "days"),
        ("durability", "water resistance", "water-resistant", "5atm", "ip68"),
        ("app ecosystem", "apps", "third-party", "wear os", "watchos"),
    )
