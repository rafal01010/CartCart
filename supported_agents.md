...
Status: Initial design artifact for review before agent implementation
...
ShoppingRunOrchestrator                                      [required-mvp]
  ...
  CategoryRouterAgent                                        [required-mvp]
    ...
    MonitorSpecialistAgent                                   [candidate-mvp]
    TechnologyDomainAnalystAgent                             [proposed-later]
      MouseSpecialistAgent                                   [proposed-later]
      AudioSpecialistAgent                                   [proposed-later]
...
It should not be a category specialist because it is useful across categories. A monitor specialist, headphone specialist, office-chair analysis path, or generic product analyst may all request YouTube-derived evidence.
...
| `MonitorSpecialistAgent` | `candidate-mvp` | Provide deeper monitor-specific analysis. | Specialist tool/sub-run | Monitor brief/products/evidence | `CategoryAnalysis` | Route to generic analysis if category mismatch or specialist fails. |
| `TechnologyDomainAnalystAgent` | `proposed-later` | Shared technology-product analysis and routing layer. | Not selected for MVP unless promoted | TBD | TBD | Generic fallback. |
...
| `AudioSpecialistAgent` | `proposed-later` | Deeper headphone/speaker analysis. | Not implemented | TBD | TBD | Technology domain or generic fallback when later approved. |
...
3. If no product/category specialist matches, routing must fall back to generic analysis rather than presenting an unsupported-category error.
4. If a specialist fails, times out, or has insufficient relevant evidence, generic analysis must remain available.
5. Domain layers are optional and should only be implemented when they improve shared reasoning or routing.
6. Source intelligence agents can be used across hierarchy levels and product categories.
7. YouTube/video evidence must remain source-backed with video IDs, channel metadata where available, timestamps when available, transcript availability status, and confidence.
8. Seller/listing trust assessment applies independently of category analysis and must be surfaced where it materially affects purchase safety.
9. Recommendation output must be verified after product, source, and trust outputs are combined.
...
3. Identify routing scope, fallback, input/output schema, permitted tools/providers, guardrails, compliance constraints, and necessary eval cases.
4. Implement or update the executable registry.
5. Implement the agent only in its dedicated checklist task.
6. Add routing tests, schema tests, trace expectations, provider fixtures, and eval cases.
7. Update status to `implemented` only after verification passes.

Examples:

- Add a Mouse Analysis Agent: place it under an implemented technology domain only if that domain exists; otherwise route it directly from `CategoryRouterAgent` and keep generic fallback.
...
- Whether `MonitorSpecialistAgent` is the first MVP product/category specialist.
- Whether `YouTubeReviewIntelligenceAgent` should be promoted from `candidate-mvp` to `required-mvp`.
- Which YouTube transcript provider strategy is acceptable: official metadata only, official captions where authorized, a third-party transcript provider, user-provided transcripts, or a combination.
- Whether a technology domain layer should remain postponed until a second technology specialist is planned.
- Whether any specialist should ever use a handoff rather than a typed tool/sub-run in MVP.
